...
1. **Pair quality** — `explore_geckoterminal(action="top_pools", network="xrpl")` plus
   an on-ledger depth check (`book_offers` + `amm_info`, hex currency codes).
   Depth survey 2026-08-16: RLUSD/XRP is the only deep, tight book (0.11% spread,
   AMM 0.21%). USDC/RLUSD is live (AMM 0.14%). BBRL-RLUSD thin (3%, AMM 1.0%).
   **USDV-XRP and OUSG-RLUSD have NO book and NO AMM on-ledger — verified issuers
   but not tradeable; re-check before quoting.**
...
The strategy's `default_config` already carries the RLUSD issuer; passing it here
explicitly keeps a hand-rolled planner call honest. Without it, amm_info is
skipped and the ceiling falls back to 0.1% instead of the pool's 0.208%.
...
If `viable: false` with the right interval → **deploy WIDEN mode** (single
level at `widen_distance_pct` / `widen_spread_fraction`). Do not stop. Only a
missing book is a hard stop. Large `divergence_vs_reference_bps` usually means
stale data, not opportunity.
...
(`binance_perpetual` delta neutral — preferred).
